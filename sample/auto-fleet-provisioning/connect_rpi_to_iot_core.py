...
parser = argparse.ArgumentParser(
    description="Connect and register a new device to the AWS IoT platform")
...
print(iot_endpoint)
root_cert_path = join(
    config["SETTINGS"]["SECURE_CERT_PATH"], config["SETTINGS"]["ROOT_CERT"])
print(root_cert_path)
private_key_path = join(
    config["SETTINGS"]["SECURE_CERT_PATH"], config["SETTINGS"]["SECURE_KEY"])
print(private_key_path)
claim_cert_path = join(
    config["SETTINGS"]["SECURE_CERT_PATH"], config["SETTINGS"]["CLAIM_CERT"])
...
long_term_credentials_path = join(
    config["SETTINGS"]["SECURE_CERT_PATH"], "permanent_cert/")
...
root_cert_long_term_path = join(
    long_term_credentials_path, config["SETTINGS"]["ROOT_CERT"])
shutil.copy2(root_cert_path, root_cert_long_term_path)

perm_config = configparser.ConfigParser()
# Maintains capitalization of variables: https://stackoverflow.com/questions/1611799/preserve-case-in-configparser
perm_config.optionxform = str
